...
3. **Evidence/processing separation.** Viewing an already-processed result must never trigger reprocessing. VIEW / ANALYZE / REANALYZE / COMPARE are distinct operations. Reprocessing creates a new `ProcessingRun`/version — it never overwrites history.
